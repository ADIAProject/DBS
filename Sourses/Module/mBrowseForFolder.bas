...
Public Function fBrowseForFolder(ByVal hWnd_Owner As Long, _
    ...
    If lpIDList Then
        ...
        If SHGetPathFromIDList(lpIDList, StrPtr(sBuffer)) Then
            ...
            fBrowseForFolder = BackslashAdd2Path(sBuffer)
        Else
    ...
    'If udtBI.lParam <> 0 Or Not IsNull(udtBI.lParam) Then
        'DebugMode "Show Open Dialog: Func - 'CoTaskMemFree(udtBI.lParam)', udtBI.lParam=" & CStr(udtBI.lParam), 2
        'Call CoTaskMemFree(udtBI.hWndOwner)
    'End If
...
Private Sub BFFSetPath(ByVal hWndDialog As Long, lpData As Long, ByVal UseStrPath As Boolean)
