...
Public Function RunAndWait(ComLine As String, DefaultDir As String, ShowFlag As VbAppWinStyle) As Boolean
    ...
    DebugMode "***RunAndWait-Start"
    DoEvents
    lngExitProc = 0

    If ShowFlag = vbHide Then
        If Not mboolHideOtherProcess Then
    ...
    DebugMode "******RunString: " & ComLine
    DebugMode "******StartDir: " & DefaultDir
    ...
    DebugMode "******ReturnCode: " & CStr(nRet) & " - " & ApiErrorText(err.LastDllError)
    CloseHandle pi.hProcess
    lngExitProc = nRet
    RunAndWait = True
    DebugMode "***RunAndWait-End"
    DoEvents
End Function

Public Sub RunUtilsShell(ByVal strPathUtils As String, _
                         Optional ByVal mboolCollectPath As Boolean = True, _
                         Optional ByVal mboolStartPathAsPathExe As Boolean = False)
    ...
    If mboolCollectPath Then
        ...
        If mboolStartPathAsPathExe Then
...
Public Function ShellEx(ByVal sFile As String, _
    ...
    If (InStr(1, sFile, ".exe", vbTextCompare) <> 0) Then
        eShowCmd = 0
    End If
